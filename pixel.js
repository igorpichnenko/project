...
const Perfetto = new function () {
  this.init = function () {
    ...
    this.bind([this.c.head, this.layer], 'dragstart', (e) => { e.preventDefault(); });
  };

  this.generateUI = function () {
    const template = '<div class="pf_head" data-pf="head">'
  ...
  this.getOptions = function () {
    if (localStorage.Perfetto === 'true') {
      const pfPos = localStorage['Perfetto.o.position'].split(',');
      const layerPos = localStorage['Perfetto.o.layer.position'].split(',');
  ...
  this.valueChange = function (e) {
    if (e.keyCode == 38) {
      ...
      } else if (e.currentTarget.name == 'pos-x' || e.currentTarget.name == 'pos-y') {
  ...
  this.proxy = function (func, args, context) {
    ...
    return function () {
      ...
      for (let i = 0; i < arguments.length; i++) if (typeof a[i] === 'undefined' || a[i] === null) a[i] = arguments[i];
